...
데이터 웨어하우스에 오늘 날짜에 해당하는 데이터를 요청하고 가공한 뒤 ElasticSearch에 적재합니다.
...
엘라스틱 서치에 적재한 데이터를 키바나를 활용하여 간편하게 시각화합니다. 자체적으로 타임스탬프 필터링 기능을 가지고 있으므로 추가적인 대시보드 작성이 불필요합니다.
...
엘라스틱 서치에 오늘 날짜에 해당하는 데이터를 요청하고 집계 작업을 실시합니다. 포털, 세부 섹션 별로 자주 등한 상위 3개 키워드를 슬랙으로 전송하비다. 만약 카운팅이 동률인 경우 모두 포함하여 전송합니다. 
