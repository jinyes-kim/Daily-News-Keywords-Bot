...
리얼 타임 서비스가 아니므로 날짜 데이터는 시간 값을 지정하지 않았습니다.
따라서 09시로 시간값이 고정됩니다.
...
엘라스틱 서치에 오늘 날짜에 해당하는 데이터를 요청하고 집계 작업을 실시합니다. 포털, 세부 섹션 별 빈출 상위 3개 키워드를 슬랙 채널에 전송합니다. 만약 카운팅이 동률인 경우 모두 포함하여 전송합니다.
