...
뉴스 기사 데이터는 날짜라는 시간적 속성을 가지며 기사 한 건이 도큐먼트라는 특징이 있습니다.
몽고 디비의 경우 도큐먼트 기반의 NoSQL 데이터베이스이고 데이터 웨어하우스이므로 빈번한 통계 작업이 요구되지 않으므로 와이드 컬럼 기반의 NoSQL 보다 적합하다고 판단했습니다.

프로젝트 초반에는 초반에는 하둡을 사용하기 위해 Hbase를 고려했으나 일일 수집 데이터 사이즈가 10MB가 채 되지 않기 때문에 오버 엔지니어링이라고 판단했습니다.
