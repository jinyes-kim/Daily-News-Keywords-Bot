...
1. 프로젝트에서 데이터를 활용하고자 하는 최종 목적은 레코드에서 "특정 키워드가 몇번 등장했는가" 입니다. 따라서 역색인을 사용하는 ES는 이러한 카운팅에 적합하다고 판단했습니다.
2. ELK 스택 학습 목적
3. 적재한 데이터를 Kibana를 통해 간편하게 시각화 할 수 있는 점이 매력적입니다.

(ES의 경우 인덱스 기반의 NoSQL이며 검색에 이점이 있는 것은 알고있지만 아직 얕은 이해도에 머물러있기 때문에 추가적으로 학습하고 정리할 예정입니다. -> 노션에 정리 완료)
