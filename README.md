...
## 1. 프로젝트 소개
뉴스 데이터를 집계하고 알림을 보내주는 서비스입니다. 매일 뉴스 데이터를 수집하고 가공한 뒤 세부 섹션 별로 자주 등장했던 키워드를 카운팅하고 대시보드 업데이트 및 알림 메시지를 전송합니다.
...
#### (2) 데이터 변환
프로듀서는 데이터를  적재하기 전에 로우 데이터를 bson  형태로 변환합니다.  만약 스키마에 적합한 형태로 변환하는 과정에서 문제가 생기는 경우 fail 항목에 추가하여 후처리가 가능합니다. 
...
도큐먼트 기반의 NoSQL 데이터베이스인 Mongo DB를 데이터 웨어하우스로 사용합니다.

뉴스 데이터의 특징은 날짜가 존재하고 기사(데이터) 한 건이 문서로 표현될 수 있습니다. 따라서 news 라는 네임으로 DB를 생성하고 날짜를 컬렉션 네임으로 설정하여 데이터 검색과 관리 측면에서의 편의를 추구했습니다.
...
수집하는 데이터는 첫 번째로 시간으로 구분될 수 있습니다. 따라서 컬렉션을 날짜로 설정하여 데이터 관리 측면에서의 편의성을 지향했습니다. 
...
데이터 웨어하우스이므로 유연하지만, 어느정도의 규격화된 스키마를 가지고 있습니다. 만약 수집한 데이터가 미리 지정한 bson 스키마에 부합하지 않는 경우 fail 리스트에 추가하여 후처리를 진행합니다.

---

## 데이터 마트 & 가공

ElasticSearch에 적재한 데이터는 Kibana를 활용하여 간편하게 시각화를 할 수 있으며 검색에 이점이 존재하는 NoSQL 데이터베이스이므로 데이터 마트로 채택했습니다.

로우 데이터를 데이터 웨어하우스에 적재한 뒤, 명사 추출기는 오늘 날짜에 해당하는 데이터를 요청해서 뉴스 제목에서 명사만 추출하여 배열로 변환하고 엘라스틱 서치에 적재합니다. (명사 추출기는 konlpy의 okt를 활용했습니다.)

가공한 데이터는 대시보드 및 알림 서비스에 활용합니다.
...
엘라스틱 서치에 적재한 데이터를 키바나를 활용하여 간편하게 시각화합니다. 자체적으로 타임스탬프 필터링 기능을 가지고 있으므로 별도로 인덱스를 생성하지 않습니다.
...
엘라스틱 서치에 오늘 날짜에 해당하는 데이터를 요청하고 집계 작업을 실시합니다. 포털, 세부 섹션 별로 자주 등한 키워드를 슬랙으로 상위 3개까지 전송하며 카운팅이 동률인 경우 모두 포함하여 전송합니다. 
...
## 스케줄링
스케줄링 툴로 Airflow를 활용했습니다. 각각의 작업은 크론을 활용할 수 있으므로 초기에는 도입 여부에 관해 고민했습니다. 그러나 최근 Airflow에 대한 관심을 가지게 되어서 학습 목적으로 활용하게 되었습니다.

Airflow를 활용하면 각 컴포넌트를 한 번에 관리할 수 있으며 각 과정의 진행 상황을 시각 이미지로 한 눈에 파악할 수 있다는 점이 매력적이었습니다.
