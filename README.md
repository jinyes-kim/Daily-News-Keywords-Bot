...
## 서비스 소개
매일 뉴스 데이터를 수집하고 가공한 뒤 세부 섹션 별로 자주 등장했던 키워드를 시각화하고 집계 결과를 메시지로 전송합니다. 
...
로우 데이터를 수집하고 데이터 웨어하우스에 적재합니다. 적재한 데이터는 가공한 뒤 데이터 마트를 업데이트하고 대시보드 갱신과 키워드 집계에 활용합니다.
...
## Load
도큐먼트 기반의 NoSQL 데이터베이스인 Mongo DB를 데이터 웨어하우스로 사용합니다. 프로젝트 초반에는 하둡을 사용하기 위해 Hbase를 고려했으나 일일 수집 데이터 사이즈가 10MB가 채 되지 않기 때문에 오버 엔지니어링이라고 판단했습니다. 또한 와이드 컬럼 기반의 NoSQL 데이터베이스의 경우 빈번한 집계 및 통계 작업에서의 이점이 존재하지만 본 프로젝트의 데이터 웨어하우스는 일일 한 번씩 데이터를 요청하는 용도이므로 별도의 이점이 없기도 했습니다. 뉴스 데이터의 특징은 날짜가 존재하고 뉴스 기사(데이터) 한 건이 문서로 표현될 수 있으므로 최종적으로는 도큐먼트 기반의 NoSQL인 Mongo DB를 선택했습니다.
...
다음뉴스의 자동생성 기사, 보도자료 같이 세부 섹션이 없는 경우는 null로 처리합니다.
...
1. 해당 프로젝트에서 데이터를 활용하고자 하는 최종 목적은 레코드에서 "특정 키워드가 몇번 등장했는가" 입니다. 따라서 역색인을 사용하는 ES는 이러한 카운팅에 적합하다고 판단했습니다.
2. ES 및 Kibana는 ELK 스택 학습 목적으로 본 프로젝트에 활용하고자 했습니다. 적재한 데이터를 Kibana를 통해 간편하게 시각화 할 수 있는 점이 매력적입니다. 

(ES의 경우 인덱스 기반의 NoSQL이며 검색에 이점이 있는 것은 알고있지만 아직 얕은 이해도에 머물러있기 때문에 추가적으로 학습하고 정리할 예정입니다. -> 정리 완료)
...
리얼 타임 서비스가 아니므로 날짜 데이터는 시간(시, 분, 초) 값을 지정하지 않았습니다. 따라서 09시로 시간값이 고정됩니다.
...
엘라스틱 서치에 적재한 데이터를 키바나를 활용하여 시각화합니다. 몽고 디비와 달리 인덱스 네이밍을 고정하고 자체적인 타임스탬프 필터링 기능을 사용해서 추가 작업 없이 업데이트합니다. 
...
또한 위 이미지에서 IT에서 등장하는 태그를 클릭하는 경우 좌측 종합 차트에서 해당 키워드가 등장하는 게시글 빈도수를 필터링할 수 있습니다. 예를들어 카카오를 선택하는 경우 최근 라이온하트 인수와 카카오페이가 상장되어서 카카오페이와 오딘 등이 관련 키워드로 함께 등장합니다. 이런 방식으로 궁금한 키워드로 범위를 좁히면서 함께 등장하는 키워드를 확인할 수 있습니다. 

현재 단순한 방식으로 키워드를 집계하고 별도의 단어 사전이 구축이 되어있지 않으므로 상위에 등장하는 키워드가 다소 아쉬운 모습을 보여주고 있습니다.
서비스가 발전한다면 뉴스 기사의 본문까지 수집하고, 자주 등장하는 키워드를 기반으로 해당 키워드가 등장하는 뉴스 기사에서 본문의 핵심을 요약하는 서비스로 발전하는 방향을 생각해볼 수 있습니다. (아쉽게도 자연어처리에 대한 이해도가 얕아서 아직 도전은 못해보고 있습니다.) 
...
ES가 검색에 최적화가 되어있다고는 하나, 이에 대해서는 자료는 찾아보고 만약 속도 측면에서 문제가 생긴다면 두 가지 방법을 활용할 수 있습니다.

1. 대시보드용 인덱스와 집계용 인덱스를 구분한다.
2. 인덱스를 하나로 사용하고자 한다면 Retention Policy를 설정하여 과부하를 피한다.

수집하는 데이터의 사이즈가 크지 않고, 가능한 자동화를 지향하므로 현재 프로젝트 성격에는 2번이 상대적으로 적합하다고 생각합니다.
만약 분리해야한다면 집계를 위한 컬럼 기반의 스토리지를 추가하는 것을 고려해볼 수 있을 것 같습니다. 


---

# Scheduling
배치 스케줄링 툴로 Airflow를 활용했습니다. 각각의 작업은 크론을 활용할 수 있으므로 초기에는 도입 여부에 관해 고민했습니다만 최근 Airflow에 대한 관심을 가지게 되어서 학습 목적으로 활용하게 되었습니다. Airflow를 활용하면 각 컴포넌트의 의존관계를 설정할 수 있으며 진행 상황을 시각 이미지로 한 눈에 파악할 수 있다는 점이 매력적입니다. 제가 사용한 2.14버전의 경우 디폴트 타임존 옵션이 제공되나 KST로 설정해도 UTC 기반으로 작동하는 문제가 있습니다.
